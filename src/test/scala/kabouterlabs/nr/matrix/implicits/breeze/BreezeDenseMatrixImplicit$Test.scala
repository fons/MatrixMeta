...
class BreezeDenseMatrixImplicit$Test extends FlatSpec  with Matchers {
  ...
  it should "commute divide and mutiply with value" in {
    ...
    assertResult(Some(true), "div1 and mul1 should commute ") {
      (((lm \\ lv) ** lv) :- div1(mul1(lm, lv),lv)).sum().map(_ < 0.000001)
    }
    //TODO : sync with eigen test
    assertResult(Some(true), "unexpected div1/mul1 commute result") {
